...
function indexedTriangleGlb(indices: [number, number, number]): Uint8Array {
  ...
  indices.forEach((value, index) =>
    view.setUint16(indexOffset + index * 2, value, true)
  );
  return glb;
}
...
describe("geoIM3D BIM/IFC import", () => {
  ...
  it("accepts only bounded WGS84 placement", () => {
    ...
    for (const candidate of [
      ...
      assert.throws(
        () => parseIfcPlacement(candidate),
        /IFC_PLACEMENT_INVALID/
      );
    }
  });
  ...
  it("validates deep GLB structure and byte cap", () => {
    ...
    for (const json of [
      {
        asset: { version: "2.0" },
        buffers: [{ uri: "https://host/model.bin", byteLength: 1 }],
      },
      { asset: { version: "2.0" }, nodes: [{ name: "IfcWall-sensitive" }] },
      {
        asset: { version: "2.0" },
        nodes: [{ extras: { GlobalId: "secret" } }],
      },
      {
        asset: { version: "2.0" },
        extensions: { vendor: { uri: "https://host" } },
      },
  ...
  it("creates only allowlisted bounded IFC summary fields", () => {
    ...
    assert.doesNotMatch(
      ...
      /private|author|globalId|sensitive/i
    );
  });
  ...
  it("builds and round-trips a self-contained scenegraph without source path", () => {
    ...
    const config = layer.metadata.vizConfig as {
      scenegraph: { modelUrl: string };
    };
    assert.match(
      config.scenegraph.modelUrl,
    ...
    assert.doesNotMatch(
      serialized,
      /hospital\.ifc|sourcePath|GlobalId|Author/i
    );
    const reopened = sanitizeIncomingIfcProject(parseProject(serialized));
    assert.deepEqual(
      reopened.layers[0].metadata.ifcImport,
      layer.metadata.ifcImport
    );
  ...
  it("rejects private scenegraphs at outbound and zero-radius boundaries", () => {
    ...
    project.layers.push(layer);
    const summary = layer.metadata.ifcImport as ReturnType<
      typeof createIfcImportSummary
    >;
    ...
    assert.equal(
      assertIfcRadiusMeters(IFC_MAX_RADIUS_METERS),
      IFC_MAX_RADIUS_METERS
    );
    ...
    assert.equal(
      sanitizeIncomingIfcProject(maximumProject).layers[0].metadata.bounds !==
        undefined,
      true
    );
  ...
  it("normalizes IFC layers at project ingress and strips foreign fields", () => {
    ...
    (layer.metadata.ifcImport as Record<string, unknown>).globalId =
      "secret-guid";
    ...
    assert.equal(
      (sanitized.layers[0].metadata.ifcImport as Record<string, unknown>)
        .globalId,
      undefined
    );
    const malicious = structuredClone(project);
    const config = malicious.layers[0].metadata.vizConfig as {
      scenegraph: { modelUrl: string };
    };
    config.scenegraph.modelUrl = "https://attacker.invalid/model.glb";
    assert.throws(
      () => sanitizeIncomingIfcProject(malicious),
      /IFC_PROJECT_INVALID/
    );
    const downgraded = structuredClone(project);
    delete downgraded.layers[0].metadata.ifcImport;
    assert.throws(
      () => sanitizeIncomingIfcProject(downgraded),
      /IFC_PROJECT_INVALID/
    );
  });
  ...
  it("keeps IFC implementation behind the Tauri compile-time boundary", () => {
    ...
    assert.match(
      hook,
      /openProjectFromShareUrl[\s\S]*sanitizeIncomingDesktopProject\([\s\S]*"share"/
    );
    assert.match(
      hook,
    ...
    const ingress = read(
      "apps/geolibre-desktop/src/lib/desktop-project-ingress.ts"
    );
    ...
    for (const path of [
      ...
      assert.match(read(path), /sanitizeRemote[A-Za-z]+Project/);
    }
    assert.match(
      read("apps/geolibre-desktop/package.json"),
      /"web-ifc": "0\.0\.77"/
    );
    assert.doesNotMatch(
      read("packages/processing/src/index.ts"),
      /web-ifc|ifc-import|IfcImport/i
    );
  });

  it("terminates the worker on cancel, close, timeout, and unmount", () => {
    const dialog = read(
      "apps/geolibre-desktop/src/components/processing/IfcImportDialog.tsx"
    );
    ...
    assert.match(
      read("apps/geolibre-desktop/src/lib/ifc-model.ts"),
      /name:\s*"IFC Model"/
    );
    assert.match(
  ...
  it("applies transforms, pre-export limits, and no raw parser logging", () => {
    ...
    const worker = read(
      "apps/geolibre-desktop/src/lib/ifc-conversion.worker.ts"
    );
    assert.match(conversion, /COORDINATE_TO_ORIGIN:\s*true/);
    assert.match(conversion, /applyMatrix4/);
    assert.match(
      conversion,
      /IFC_MAX_(?:ELEMENTS|VERTICES|INDICES|GEOMETRY_BYTES)/
    );
    assert.match(
      conversion,
