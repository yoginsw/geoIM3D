...
| 작업 Branch | `feat/geoim3d-default-ux` |
| Main 반영 | 아직 미병합 |
...
|---|---|---|---|
| 0. Baseline/Inventory | **부분 완료** | Inventory와 Baseline 결과 존재. 금지 문자열 검사 Script와 Private Repository Snapshot 절차 누락 | `docs/rebranding/01_BRAND_INVENTORY.md` |
...
기준일: **2026-07-17**, Phase 2 완료 상태.

| Gate | 최신 확인 결과 |
|---|---:|
| Product/Target Contract | 71 passed |
| Frontend Coverage | Lines 82.46%, Branches 83.62%, Functions 68.50% |
...
- 이 Baseline은 Source/Browser와 Cargo Check 증거다. Windows 설치 Package Smoke 증거가 아니다.
...
### Sprint H0 — Phase 0 잔여 산출물 종료

**목적:** 이후 모든 Phase에서 자동으로 Brand 회귀와 안전한 Repository 이전 절차를 검증한다.

작업:

- [ ] `scripts/check-geoim3d-brand.mjs` 추가
- [ ] About/License/Attribution Allowlist를 제외한 사용자 노출 `GeoLibre` 검사
- [ ] Source 내부 식별자와 사용자 노출 문자열을 구분한 Fixture/Test 추가
- [ ] `docs/operations/PRIVATE_REPOSITORY_SNAPSHOT.md` 작성
- [ ] Baseline 명령과 결과 갱신

Acceptance:

- [ ] Script가 의도된 사용자 노출 위반 Fixture를 실패시킨다.
- [ ] 현재 Source는 Allowlist 정책으로 통과한다.
- [ ] Snapshot 절차가 `.git` 삭제/History Rewrite 없이 수행되도록 문서화된다.
- [ ] Dependency 추가 없음.

검증:

```bash
node scripts/check-geoim3d-brand.mjs
npm run lint
npm run build
...
- [ ] 실제 Windows Desktop에서 Source 실행 및 핵심 2D/3D 시작 Smoke를 재확인한다.
- [ ] Linux/WSL 검증만으로 Phase 0 완료 처리하지 않는다.
...
### Sprint 3A — Phase 3 Project Identity Contract
...
- [ ] `ensureProjectFileName()`이 신규 파일을 `.geoim3d.json`으로 생성한다.
- [ ] Canonical Open/Save/Save As/Recent/Drag-and-drop가 `.geoim3d.json`만 노출·허용한다.
- [ ] 승인 전 `.geolibre.json`/`.geolibre` Legacy Import가 노출되지 않는다.
- [ ] 일반 `.json`은 Desktop 로컬 파일 읽기 Allowlist에서 계속 거부한다.
- [ ] Malformed Project와 Credential 포함 Project 정책을 검증한다.
- [ ] Round-trip 후 Layer/Map View/Plugin State가 보존된다.
...
- [ ] Open/Save/Recent/Drag-and-drop가 `.geoim3d.json`에서 동작한다.
- [ ] Legacy GeoLibre 확장자는 승인 전 Product Open Filter와 Import UX에 나타나지 않는다.
- [ ] Project Schema Version과 External Plugin API는 변경되지 않는다.
- [ ] Project 파일에 Credential이 없다.
...
```bash
git switch -c chore/geoim3d-phase0-closure
```
