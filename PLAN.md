...
| 항목                 | 현재 상태                                                                                 |
| -------------------- | ----------------------------------------------------------------------------------------- |
| Root                 | `/home/nurig/projects/GeoLibre-phase7e`                                                   |
| 작업 Branch          | `feat/geoim3d-3d-scene-preset`                                                           |
...
| 5. VWorld Built-in Plugin  | **완료**      | Windows Desktop 전용 Private Transport, 공식 API/약관·보안·저장·Attribution, Contract Test 및 Native Runtime 완료                                                   | `docs/rebranding/05_VWORLD_ACCEPTANCE.md`         |
| 6. Windows/Web Packaging   | **부분 완료** | Bundle ID/Update 제거/PWA/Installer·Portable Install·Save·Open·Uninstall 완료. OS Association·공개 자동 배포 제외. Docker Runtime·signed MSIX는 외부 환경 Gate 대기 | `apps/geolibre-desktop/src-tauri/tauri.conf.json` |
| 7. 3D 업무 기능 강화       | **진행 중**   | Phase 7A CAD/GIS, 7B BIM/IFC, 7C 토공량/절성토, 7D1 경사·안전 완료. 7D2 품질·artifact·Windows Runtime Gate 완료, final exact-stage 3관점 Review 진행 중             | `docs/rebranding/07D2_VIEWSHED_ANALYSIS.md`       |
...
기준일: **2026-07-21**, Phase 7D2 final exact-stage Review 직전 검증 상태.

| Gate                                  |                                                                                    최신 확인 결과 |
| ------------------------------------- | ------------------------------------------------------------------------------------------------: |
| Brand Contract                        |                                                                                              PASS |
| Credential Architecture Target        |                                                                                         19 passed |
| Frontend Full Suite                   |                                                                 3,037 passed, 1 skipped, 0 failed |
| Frontend Coverage                     |                                                   Lines 82.05%, Branches 82.19%, Functions 70.32% |
...
| Playwright Full Suite                 |                                                                                         28 passed |
...
| Windows Rust Full Suite               |                                                                                         69 passed |
| Windows Credential Manager Round-trip |                                                                                              통과 |
| Phase 7D2 Pre-Review Gitleaks         |                                                                                  76 files, Leak 0 |
| Windows Native Tauri                  | 7D2 EPSG:5179/5186 Picker→Worker→Preview→Apply, EPSG:5186 Save As→Exit→Open, Console/Page Error 0 |
| Windows Build/Bundle                  |            Debug/Release EXE, MSI administrative extraction, NSIS install/run/open/uninstall 통과 |
...
- [x] Windows Installer/Portable Install·Run·Save·Open·Uninstall
  - Installer Install/Native Renderer/직접 Project Open/Uninstall 검증됨
  - Portable Extract/Run/Project Open/Registry 비변경 검증됨
  - Phase 7C exact source NSIS 격리 Install→Canonical Open→Native Save As→Uninstall 완료
- [x] Web PWA Offline
...
Historical Phase 7C Windows Artifact Runtime 증거(현재 Phase 8 artifact 판정에 재사용하지 않음):

- Phase 7C NSIS: Install/Startup Argument Open/Launch/Uninstall PASS, File Association/ProgID 미등록
- Phase 7C Portable: Extract/Startup Argument Open/Launch PASS, Registry 비변경
- Historical `geoIM3D_1.0.0_x64-setup.exe` SHA256:
  `E1EE58D03BA1B1C9E55F84F91ECC7424543988F8BB366577DA53345B4344DF1E`
- Historical `geoIM3D-1.0.0-x64-portable.zip` SHA256:
...
- [x] BIM/IFC Import Adapter — **완료 / Security·Windows·3D 최종 독립 Review 승인**
  - Branch: `feat/geoim3d-bim-ifc-import`
  - Commit: `03266367cb110d61173e767ab7082d4e7fd6cd14`
  - Windows Desktop 전용 Local IFC → self-contained GLB MVP
  - Dedicated Worker, bounded STEP/GLB/geometry, actual index 검증 및 취소/Timeout
  - 모든 Project ingress 재검증, private scenegraph inbound/outbound 차단
  - 요구사항/Acceptance: `docs/rebranding/07B_BIM_IFC_IMPORT.md`
...
- [ ] 경사·가시권·안전 분석 — **Phase 7D1 완료 / Phase 7D2 품질·Windows Runtime 완료, final exact-stage Review 중**
  - 7D1 Branch/Commit: `feat/geoim3d-terrain-safety-analysis` / `843305a349fc84e2d198a29a0d8aa1f949298e26`
  - 7D1: Local DEM + AOI 기반 Horn 3×3 경사 및 사용자 임계값 안전 Summary 완료
  - 7D2 Branch: `feat/geoim3d-viewshed-analysis`
  - 7D2: Local DEM + AOI + Observer 기반 bounded planar cell-column 가시권
  - Acceptance: `docs/rebranding/07D1_TERRAIN_SLOPE_SAFETY.md`, `docs/rebranding/07D2_VIEWSHED_ANALYSIS.md`
- [ ] 3D Scene Project Preset — **Phase 7E Runtime 구현·Windows smoke 완료 / scenario 기반 Memory Gate는 2026-07-22 사용자 지시로 deferred**
- [ ] 환경·재난 Dashboard

### Phase 8 — Release 1.0 — **진행 중**
...
```bash
cd /home/nurig/projects/GeoLibre-phase7e
...
```bash
git switch feat/geoim3d-3d-scene-preset
```
