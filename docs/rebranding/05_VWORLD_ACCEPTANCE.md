...
기준일: **2026-07-18**
상태: **공식 문서 조사 및 구현 전 Acceptance 초안**
...
| [도시지역 상세](https://www.vworld.kr/dev/v4dv_2ddataguide2_s002.do?svcIde=uq111) | 2026-07-18 | `LT_C_UQ111` | Service ID 근거; Field Contract 확인 후 활성화 |
| [관리지역 상세](https://www.vworld.kr/dev/v4dv_2ddataguide2_s002.do?svcIde=uq112) | 2026-07-18 | `LT_C_UQ112` | Service ID 근거; Field Contract 확인 후 활성화 |
| [농림지역 상세](https://www.vworld.kr/dev/v4dv_2ddataguide2_s002.do?svcIde=uq113) | 2026-07-18 | `LT_C_UQ113` | Service ID 근거; Field Contract 확인 후 활성화 |
| [자연환경보전지역 상세](https://www.vworld.kr/dev/v4dv_2ddataguide2_s002.do?svcIde=uq114) | 2026-07-18 | `LT_C_UQ114` | Service ID 근거; Field Contract 확인 후 활성화 |
...
| 구분 | Contract |
|---|---|
| 공식 URL | `https://api.vworld.kr/req/wmts/1.0.0/{key}/{layer}/{z}/{y}/{x}.png` |
| 1차 Layer Allowlist | `Base`, `white`, `midnight`, `Hybrid` |
| Zoom | `Base`/`Hybrid`: 6~19, `white`/`midnight`: 6~18 |
| Format | PNG |
| Frontend URL | `geoim3d-vworld://tile/{layer}/{z}/{x}/{y}` — 실제 Key 없음 |
| 응답 | Rust가 검증한 PNG Byte와 Media Type만 반환 |

`Satellite`는 일반 Tile URL이 아니라 `Satellite/themes/{category}/{year}/{city}/{tileMatrix}/{tileRow}/{tileCol}.{tileType}` 형식과 별도 Metadata Allowlist가 필요하다. `category`/`year`/`city` 공식 값과 운영 이용 범위를 고정하기 전에는 Menu, Layer Allowlist, Command에서 모두 제외한다.
...
용도지역 Service별 Field/Properties는 각 공식 상세 Reference를 RED Fixture로 고정한 후 활성화한다. 건물 Service ID와 Field Contract는 **공식 Reference에서 추가 확인 필요**이며 확인 전에는 구현하지 않는다.
