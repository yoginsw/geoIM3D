...
export const TERRAIN_SAFETY_WORKER_PEAK_BUDGET_BYTES = 128 * 1024 * 1024;
const PROJECTED_GEOMETRY_AND_METADATA_RESERVE_BYTES = 8 * 1024 * 1024;
const HORN_WORKING_RESERVE_BYTES = 1 * 1024 * 1024;
...
function numbers(value: unknown, maxItems: number): number[] {
  ...
  if (Array.isArray(value) || ArrayBuffer.isView(value)) {
    ...
    if (source.length > maxItems) fail("TERRAIN_SAFETY_LIMIT_EXCEEDED");
    const result = new Array<number>(source.length);
    for (let index = 0; index < source.length; index += 1) {
      const item = source[index];
      if (!finite(item)) fail("TERRAIN_SAFETY_TIFF_INVALID");
      result[index] = item;
    }
    return result;
  }
  fail("TERRAIN_SAFETY_TIFF_INVALID");
}
...
function assertMaterializedMetadataBound(directory: FileDirectory): number {
  ...
  for (const map of maps) {
    if (!map) continue;
    count += map.size;
    if (count > 256) fail("TERRAIN_SAFETY_LIMIT_EXCEEDED");
    for (const [key, value] of map) {
      ...
      if (size > MAX_METADATA_BYTES) fail("TERRAIN_SAFETY_LIMIT_EXCEEDED");
    }
  }
  return size;
}
...
function assertClassicTiff(bytes: ArrayBuffer): void {
  if (bytes.byteLength < 8 || bytes.byteLength > TERRAIN_SAFETY_MAX_INPUT_BYTES) {
    fail(bytes.byteLength > TERRAIN_SAFETY_MAX_INPUT_BYTES
...
function checkedProduct(left: number, right: number): number {
  ...
  if (!Number.isSafeInteger(value)) fail("TERRAIN_SAFETY_LIMIT_EXCEEDED");
  return value;
}
...
function materializeIfdNumbers(
  ...
  if (descriptor.type !== 3 && descriptor.type !== 4) fail("TERRAIN_SAFETY_TIFF_INVALID");
  const valueOffset = descriptor.fieldBytes <= 4
  ...
  for (let index = 0; index < descriptor.count; index += 1) {
    values[index] = descriptor.type === 3
  ...
  if (
    ...
    checkedProduct(values.length, descriptor.type === 3 ? 2 : 4) !== descriptor.fieldBytes ||
...
function preflightBoundedClassicIfd(bytes: ArrayBuffer): ClassicIfdPreflight {
  ...
  if (entryCount < 1 || entryCount > 256) fail("TERRAIN_SAFETY_LIMIT_EXCEEDED");
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (tags.has(tag) || !typeBytes || count < 1) fail("TERRAIN_SAFETY_TIFF_INVALID");
    tags.add(tag);
    ...
    if (tag === 42112) {
      if (type !== 2) fail("TERRAIN_SAFETY_TIFF_INVALID");
      gdalMetadataDescriptor = descriptor;
    }
    if ((tag === 273 || tag === 279) && count > MAX_STRIPS) {
      fail("TERRAIN_SAFETY_LIMIT_EXCEEDED");
    }
    if (tag === 330) fail("TERRAIN_SAFETY_TIFF_INVALID");
    if (tag === 254) {
      if (count !== 1 || (type !== 3 && type !== 4)) fail("TERRAIN_SAFETY_TIFF_INVALID");
      const value = type === 3
        ? view.getUint16(offset + 8, littleEndian)
        : view.getUint32(offset + 8, littleEndian);
      if (value !== 0) fail("TERRAIN_SAFETY_TIFF_INVALID");
    }
    if (tag === 255) fail("TERRAIN_SAFETY_TIFF_INVALID");
  }
  if (
  ...
  const stripOffsets = materializeIfdNumbers(view, littleEndian, stripOffsetsDescriptor);
  const stripByteCounts = materializeIfdNumbers(view, littleEndian, stripByteCountsDescriptor);
  let gdalMetadata: string | null = null;
  if (gdalMetadataDescriptor) {
    const valueOffset = gdalMetadataDescriptor.fieldBytes <= 4
      ? gdalMetadataDescriptor.entryOffset + 8
      : view.getUint32(gdalMetadataDescriptor.entryOffset + 8, littleEndian);
    const raw = new Uint8Array(bytes, valueOffset, gdalMetadataDescriptor.fieldBytes);
    if (raw.byteLength !== gdalMetadataDescriptor.fieldBytes) {
      fail("TERRAIN_SAFETY_TIFF_INVALID");
    }
    const end = raw[raw.length - 1] === 0 ? raw.length - 1 : raw.length;
    try {
      gdalMetadata = new TextDecoder("utf-8", { fatal: true }).decode(raw.subarray(0, end));
...
function assertStripRanges(
  ...
  for (let index = 1; index < ranges.length; index += 1) {
    if (ranges[index][0] < ranges[index - 1][1]) fail("TERRAIN_SAFETY_TIFF_INVALID");
  }
}
...
function decodeUncompressedSamples(
  ...
  else fail("TERRAIN_SAFETY_SAMPLE_UNSUPPORTED");

  const view = new DataView(bytes);
  const bytesPerSample = bits / 8;
  let outputIndex = 0;
  for (let stripIndex = 0; stripIndex < preflight.stripOffsets.length; stripIndex += 1) {
    const offset = preflight.stripOffsets[stripIndex];
    const count = preflight.stripByteCounts[stripIndex];
    for (let position = offset; position < offset + count; position += bytesPerSample) {
      let value: number;
      ...
      if (outputIndex >= pixels) fail("TERRAIN_SAFETY_TIFF_INVALID");
...
export function estimateTerrainSafetyWorkerPeakBytes(
  inputBytes: number,
  decodedBytes: number,
  ...
  if (
    values.some((value) => !Number.isSafeInteger(value) || value < 0) ||
    inputBytes > TERRAIN_SAFETY_MAX_INPUT_BYTES ||
  ...
  const peak =
    ...
    HORN_WORKING_RESERVE_BYTES +
    PARSER_AND_RUNTIME_RESERVE_BYTES;
  if (!Number.isSafeInteger(peak) || peak > TERRAIN_SAFETY_WORKER_PEAK_BUDGET_BYTES) {
    fail("TERRAIN_SAFETY_LIMIT_EXCEEDED");
  }
  return peak;
}

export async function decodeTerrainSafetyGeoTiff(
  bytes: ArrayBuffer,
): Promise<TerrainSafetyRaster> {
  assertClassicTiff(bytes);
  const preflight = preflightBoundedClassicIfd(bytes);
  try {
    const tiff = await fromArrayBuffer(bytes);
    if ((await tiff.getImageCount()) !== 1) fail("TERRAIN_SAFETY_TIFF_INVALID");
    ...
    if (
      ...
      pixels > TERRAIN_SAFETY_MAX_PIXELS
    ) {
      fail("TERRAIN_SAFETY_LIMIT_EXCEEDED");
    }
    ...
    if (planar.length !== 1 || planar[0] !== 1) fail("TERRAIN_SAFETY_SAMPLE_UNSUPPORTED");
    ...
    if (rowsPerStrip.length !== 1) fail("TERRAIN_SAFETY_TIFF_INVALID");
    assertStripRanges(
      offsets,
      counts,
      bytes.byteLength,
    ...
    if (error instanceof Error && /^TERRAIN_SAFETY_[A-Z_]+$/.test(error.message)) throw error;
    fail("TERRAIN_SAFETY_TIFF_INVALID");
  }
}
