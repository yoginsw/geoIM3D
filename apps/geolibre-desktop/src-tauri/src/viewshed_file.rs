...
const VIEWSHED_MAX_BYTES: u64 = 48 * 1024 * 1024;
const FILE_UNSUPPORTED: &str = "VIEWSHED_FILE_UNSUPPORTED";
const FILE_TOO_LARGE: &str = "VIEWSHED_FILE_TOO_LARGE";
const FILE_CHANGED: &str = "VIEWSHED_FILE_CHANGED";
const FILE_UNREADABLE: &str = "VIEWSHED_FILE_UNREADABLE";
const TIFF_INVALID: &str = "VIEWSHED_TIFF_INVALID";
...
fn file_identity(file: &std::fs::File) -> Result<FileIdentity, String> {
    ...
    if succeeded == 0 {
        return Err(FILE_UNREADABLE.to_string());
    }
    Ok(FileIdentity {
    ...
    F: FnOnce(),
{
    ...
    let maximum = VIEWSHED_MAX_BYTES as usize;
    ...
    let final_metadata = file.metadata().map_err(|_| FILE_CHANGED.to_string())?;
    let final_identity = file_identity(&file).map_err(|_| FILE_CHANGED.to_string())?;
...
#[cfg(target_os = "windows")]
#[tauri::command]
pub async fn pick_and_read_viewshed_geotiff(
    app: tauri::AppHandle,
    ...
    let selected = receiver
        .await
        .map_err(|_| FILE_UNREADABLE.to_string())?
        .ok_or_else(|| "VIEWSHED_CANCELLED".to_string())?;
    let path = selected
        .into_path()
        .map_err(|_| FILE_UNSUPPORTED.to_string())?;
...
mod tests {
    ...
    fn rejects_initially_oversized_file_before_reading_payload() {
        ...
        file.set_len(VIEWSHED_MAX_BYTES + 1).unwrap();
        drop(file);
    ...
    fn rejects_file_symlink_reparse_points_when_supported() {
        ...
        if symlink_file(&target, &link).is_ok() {
            assert_eq!(read_bounded(&link).unwrap_err(), FILE_UNSUPPORTED);
    ...
    fn accepts_exact_maximum_size() {
        ...
        file.set_len(VIEWSHED_MAX_BYTES).unwrap();
    ...
    fn rejects_path_replacement_after_initial_metadata() {
        ...
        assert_eq!(result.unwrap_err(), FILE_CHANGED);
        let _ = std::fs::remove_file(path);
