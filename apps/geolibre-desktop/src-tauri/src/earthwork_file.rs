...
const EARTHWORK_MAX_BYTES: u64 = 48 * 1024 * 1024;
const FILE_INVALID: &str = "EARTHWORK_FILE_INVALID";
const FILE_TOO_LARGE: &str = "EARTHWORK_FILE_TOO_LARGE";
const FILE_READ_FAILED: &str = "EARTHWORK_FILE_READ_FAILED";
const TIFF_INVALID: &str = "EARTHWORK_TIFF_INVALID";
    ...
    F: FnOnce(),
{
    ...
    if initial.len() > EARTHWORK_MAX_BYTES {
    ...
    let maximum = EARTHWORK_MAX_BYTES as usize;
...
#[tauri::command]
pub fn read_earthwork_geotiff(path: String) -> Result<tauri::ipc::Response, String> {
    if !has_allowed_extension(Path::new(&path)) {
        return Err(FILE_INVALID.to_string());
    }
...
mod tests {
    ...
    fn rejects_initially_oversized_file_before_reading_payload() {
        ...
        file.set_len(EARTHWORK_MAX_BYTES + 1).unwrap();
        drop(file);
    ...
    fn accepts_exact_maximum_size() {
        ...
        file.set_len(EARTHWORK_MAX_BYTES).unwrap();
